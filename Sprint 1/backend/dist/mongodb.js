...
class MongoDB {
    ...
    async registerNewUser(username, password) {
        ...
        try {
            ...
            if (existingUser) {
                console.warn(`Registration rejected: Email ${username} already exists.`);
                return false;
            }
        }
        catch (error) {
        ...
        try {
            ...
            const authUpdateResult = await this.getAuthCollection().updateOne({ type: "userdef" }, {
                $set: {
            ...
            if (authUpdateResult.modifiedCount === 1) {
                const userProfileInsertResult = await this.getUsersCollection().insertOne({
    ...
    async updateUserProfile(email, data) {
        ...
        const updateResult = await this.getUsersCollection().updateOne({ email: email }, { $set: updatePayload }, { upsert: false } // Profile should already exist
        );
